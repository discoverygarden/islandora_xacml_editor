...
class XacmlParser {
  ...
  /**
   * Parses a XML string containing XACML, returns array representing XACML.
   *
   * @param string $xacml_string
   *   String containing the source XACML.
   *
   * @throws XacmlException
   *   Thrown when the passed in policy can't be parsed. Particularly when the
   *   policy was not generated by the islandora XACML editor.
   *
   * @return array
  ...
  protected static function findMethods(array &$rule, DOMElement $element) {
    ...
    foreach ($values as $value) {
      ...
      if (preg_match('/api-a/', $method) || preg_match('/api-m/', $method)) {
        $rule['methods'][] = Unicode::substr($method, 35);
      }
      else {
        // Methods are returned as they are represented in XACML
        // for example:
        // urn:fedora:names:fedora:2.1:action:id-getDatastreamDissemination
        // Here we split off the first 38 characters, which for this example
        // would give us: getDatastreamDissemination.
        $rule['methods'][] = Unicode::substr($method, 38);
      }
    }
  }
